...
const SUGGESTIONS = [
  ...
  'Analyseer mijn uitgaven',
  'Voeg €50 boodschappen toe vandaag',
...
export default function AIChatbot() {
  ...
  const [msgs, setMsgs] = useState<ChatMsg[]>([
    {
      role: 'ai',
      text: 'Hoi! Ik ben Family AI. Hoe kan ik je helpen?',
  ...
  async function runTool(name: string, toolInput: Record<string, unknown>) {
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return { error: 'Niet ingelogd' };
    const now = new Date();
    const m = ((toolInput.month as number) ?? now.getMonth() + 1) - 1;
    ...
    if (name === 'add_transaction') {
      const tx = {
        ...
        type: (toolInput.amount as number) > 0 ? 'income' : 'expense',
        description: toolInput.description,
        category: toolInput.category,
        date: (toolInput.date as string) || now.toISOString().split('T')[0],
        source: 'AI',
        status: 'OK',
        is_zakelijk: toolInput.is_zakelijk || false,
      };
    ...
    if (name === 'get_budget_status') {
      ...
      const mTx = (tx || []).filter((t: Record<string, unknown>) => {
      ...
      return (bud || []).map((b: Record<string, unknown>) => {
        const sp = mTx
          .filter((t: Record<string, unknown>) => t.category === b.category)
          .reduce((s: number, t: Record<string, unknown>) => s + (t.amount as number), 0);
        return {
          category: b.category,
          budget: b.monthly_limit,
          spent: sp,
          remaining: (b.monthly_limit as number) - sp,
        };
      });
    }
    ...
    if (name === 'get_summary') {
      ...
      const exp = mTx
        ...
        .reduce((s: number, t: Record<string, unknown>) => s + (t.amount as number), 0);
      return {
        month: now.toLocaleString('nl-NL', { month: 'long', year: 'numeric' }),
        income: inc,
        expenses: exp,
        net: inc - exp,
        count: mTx.length,
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      const systemPrompt = buildSystemPrompt('Abdul', 'vriend', {
        totaalSaldo: 0, inkomsten: 0, uitgaven: 0, schulden: 0, netto: 0,
      });
      let resp = await callClaude(k, systemPrompt, currentMsgs as Parameters<typeof callClaude>[2]);
      ...
      while (resp.stop_reason === 'tool_use') {
        ...
        setHistory(currentMsgs);
        resp = await callClaude(k, systemPrompt, currentMsgs as Parameters<typeof callClaude>[2]);
      }

      const replyText = resp.content?.find((c: Record<string, unknown>) => c.type === 'text')?.text as string || 'Geen antwoord';
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ boxShadow: '0 4px 20px rgba(1,121,254,.5)' }}
      ...
      {open && (
        <div
          className="fixed bottom-24 right-6 z-50 w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center gap-3 p-4 border-b border-gray-100">
            ...
            <div>
              <p className="text-sm font-bold">Family AI</p>
                ...
                <div>
                  <div
                    className={`px-4 py-2.5 rounded-xl text-sm ${
                      m.role === 'user' ? 'gradient-blue text-white' : 'bg-gray-100 text-gray-800'
                    }`}
                    style={{ borderRadius: m.role === 'ai' ? '4px 12px 12px 12px' : '12px 4px 12px 12px' }}
                  >
                    {m.text}
                  </div>
                  <p className={`text-xs text-gray-300 mt-1 ${m.role === 'user' ? 'text-right' : ''}`}>
                    {m.time}
                  </p>
                </div>
              </div>
            ...
            <div className="px-4 pb-2 flex flex-wrap gap-1.5">
              {SUGGESTIONS.map(s => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 border border-gray-200 rounded-full text-gray-500 hover:border-blue-400 hover:text-blue-600 transition"
                >
                  {s}
                </button>
              ))}
      ...
      <style>{`
        ...
        @keyframes pulse {
          0%,  100% { box-shadow: 0 4px 20px rgba(1,121,254,.5); }
          50%        { box-shadow: 0 4px 30px rgba(1,121,254,.8); }
        }
      `}</style>
    </>
  );
