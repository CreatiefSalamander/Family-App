...
const DEFAULT_BUDGETS = [
  { category: 'Wonen', monthly_limit: 290 },
  { category: 'Boodschappen', monthly_limit: 500 },
  { category: 'Transport', monthly_limit: 150 },
  { category: 'Vrije tijd', monthly_limit: 150 },
  { category: 'Zorgverzekering', monthly_limit: 180 },
  { category: 'Telefoon', monthly_limit: 140 },
];
...
export default function BegrotingPage() {
  ...
  const mTx = transacties.filter(t => {
    const d = new Date(t.date);
    return d.getMonth() === maand.getMonth() && d.getFullYear() === maand.getFullYear() && t.type === 'expense';
  });
  ...
  const activeBudgets = budgetten.length > 0
    ? budgetten
    : DEFAULT_BUDGETS.map((b, i) => ({ ...b, id: String(i), user_id: '' }));

  return (
    <div className="px-7 py-6">
      <div className="flex items-center gap-4 mb-6">
        <button
          onClick={() => { const d = new Date(maand); d.setMonth(d.getMonth() - 1); setMaand(d); }}
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 gradient-blue text-white px-4 py-2 rounded-lg text-sm font-semibold"
        >
      ...
      <div className="card p-6 mb-6">
        <div className="flex justify-between mb-2">
          <div>
            ...
            <p className="font-mono text-2xl font-bold">{fmt(totB)}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-400 mb-1">Besteed</p>
            <p className={`font-mono text-2xl font-bold ${pct > 90 ? 'text-red-500' : pct > 70 ? 'text-amber-500' : 'text-green-600'}`}>
              {fmt(totS)}
            </p>
          </div>
        </div>
          ...
          return (
            <div key={b.category} className="card p-5 hover:border-blue-200 cursor-pointer transition-all">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-xs font-bold ${bp > 90 ? 'text-red-500' : bp > 70 ? 'text-amber-500' : 'text-green-600'}`}>
                  {Math.round(bp)}%
                </span>
              </div>
              <p className="text-xs text-gray-400 mb-2">{fmt(sp)} van {fmt(b.monthly_limit)}</p>
              <div className="h-2 bg-gray-100 rounded-full">
                <div className={`h-full rounded-full ${color}`} style={{ width: bp + '%' }} />
              </div>
            </div>
          );
        })}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-lg">Budget instellen</h2>
              <button onClick={() => setShowAdd(false)}><X size={20} /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-sm font-medium block mb-1">Categorie</label>
                <select
                  value={form.category}
              ...
              <div>
                <label className="text-sm font-medium block mb-1">Maandlimiet (€)</label>
                <input
                  type="number"
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={saveBudget} className="flex-1 gradient-blue text-white rounded-lg py-2.5 text-sm font-semibold">
                Opslaan
              </button>
            </div>
          </div>
