    ...
    <div className="px-7 py-6">
      ...
      {doelen.length === 0 ? (
        <div className="card p-16 text-center text-gray-400">
          ...
          <p className="font-semibold text-gray-600">Maak je eerste spaardoel aan</p>
          <button
            onClick={() => setShowAdd(true)}
            className="mt-4 gradient-blue text-white px-6 py-2.5 rounded-lg text-sm font-semibold"
          >
            Toevoegen
          </button>
        </div>
              ...
              <div key={g.id} className="card p-6 text-center hover:border-blue-200 transition-all">
                ...
                {daysLeft !== null && (
                  <p className="text-xs text-gray-400 mb-3">
                    {daysLeft > 0 ? `${daysLeft} dagen resterend` : 'Deadline verstreken'}
                  </p>
                )}
                <button
                  onClick={() => setShowDep(g.id)}
                  className="w-full gradient-blue text-white py-2.5 rounded-lg text-sm font-semibold hover:opacity-90"
                >
                  + Storting
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-lg">Nieuw doel</h2>
            ...
            <div className="space-y-3">
              <div className="flex gap-3">
                ...
                <div className="w-24">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-center focus:outline-none"
                    maxLength={2}
                  />
                </div>
              </div>
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-sm font-medium block mb-1">Doelbedrag (€)</label>
                  <input
                    type="number"
                ...
                <div className="flex-1">
                  <label className="text-sm font-medium block mb-1">Huidig (€)</label>
                  <input
                    type="number"
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={saveDoel} className="flex-1 gradient-blue text-white rounded-lg py-2.5 text-sm font-semibold">
                Opslaan
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowDep(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <input
              ...
              className="w-full border border-gray-200 rounded-lg px-4 py-3 text-lg font-mono font-bold focus:outline-none focus:border-blue-400 mb-4"
            />
            <div className="flex gap-3">
              <button onClick={() => setShowDep(null)} className="flex-1 border border-gray-200 rounded-lg py-2.5 text-sm font-semibold">
                Annuleren
              </button>
              <button onClick={saveDeposit} className="flex-1 gradient-blue text-white rounded-lg py-2.5 text-sm font-semibold">
                Opslaan
              </button>
            </div>
          </div>
