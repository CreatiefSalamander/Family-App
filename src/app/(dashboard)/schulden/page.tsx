...
export default function SchuldenPage() {
  ...
  const displayed = schulden.length > 0 ? schulden : INIT_SCHULDEN.map((s,i) => ({...s, id:String(i), user_id:'', notitie:undefined, regeling:undefined, start_datum:undefined, einde_datum:undefined}));
  ...
  return (
    <div className="px-7 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={()=>setShowAdd(true)} className="flex items-center gap-2 gradient-blue text-white px-4 py-2 rounded-lg text-sm font-semibold"><Plus size={16}/>Toevoegen</button>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px_80px] gap-2 px-5 py-3 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wide">
          <span>Schuldeiser</span><span>Oorspronkelijk</span><span>Restant</span><span>Termijn/mnd</span><span>%</span><span>Status</span>
        </div>
        {displayed.map((s,i) => {
          const rest = s.oorspronkelijk - s.afgelost;
          const pct = s.oorspronkelijk > 0 ? Math.round((s.afgelost/s.oorspronkelijk)*100) : 0;
          return (
            <div key={s.id||i} className="grid grid-cols-[2fr_1fr_1fr_1fr_80px_80px] gap-2 px-5 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 items-center">
              <div>
                <p className="font-semibold text-sm">{s.schuldeiser}</p>
                <p className="text-xs text-gray-400">{s.type}</p>
                <div className="mt-1.5 h-1.5 bg-gray-100 rounded-full w-32"><div className="h-full bg-green-500 rounded-full" style={{width:pct+'%'}} /></div>
              </div>
              ...
              <span className={\`text-xs px-2 py-1 rounded-full font-medium \${s.status==='Actief'?'bg-blue-50 text-blue-600':s.status==='Afbetaald'?'bg-green-50 text-green-600':'bg-red-50 text-red-600'}\`}>{s.status}</span>
            </div>
          );
        })}
      </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={()=>setShowAdd(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h2 className="font-bold text-lg">Schuld toevoegen</h2><button onClick={()=>setShowAdd(false)}><X size={20}/></button></div>
            <div className="space-y-3">
              {[['schuldeiser','Schuldeiser','Bijv. DUO, Belastingdienst'],['type','Type','Lening, Betalingsregeling...'],['oorspronkelijk','Oorspronkelijk bedrag (€)','26200'],['afgelost','Afgelost (€)','0'],['maandtermijn','Maandtermijn (€)','300']].map(([k,l,p]) => (
                <div key={k}><label className="text-sm font-medium block mb-1">{l}</label>
                  <input type={['oorspronkelijk','afgelost','maandtermijn'].includes(k)?'number':'text'} placeholder={p} value={(form as Record<string,string>)[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-400" /></div>
              ))}
            </div>
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={saveSchuld} className="flex-1 gradient-blue text-white rounded-lg py-2.5 text-sm font-semibold">Opslaan</button>
            </div>
          </div>
        </div>
      )}
