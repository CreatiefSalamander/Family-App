...
export default function JaaroverzichtPage() {
  ...
  const totInkomsten = chartData.reduce((s, r) => s + r.Inkomsten, 0);
  const totUitgaven = chartData.reduce((s, r) => s + r.Uitgaven, 0);
  const totNetto = chartData.reduce((s, r) => s + r.Netto, 0);
    ...
    <div className="px-7 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="w-9 h-9 border border-gray-200 rounded-lg flex items-center justify-center hover:bg-gray-50 text-sm"
          >
            ←
          </button>
          <button
      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-[120px_1fr_1fr_1fr_1fr] gap-2 px-5 py-3 bg-gray-50 text-xs font-semibold text-gray-400 uppercase tracking-wide">
          ...
          <span>Inkomsten</span>
          <span>Uitgaven</span>
          <span>Aflossingen</span>
          <span>Netto</span>
        </div>
        {chartData.map(row => (
          <div
            key={row.naam}
            className="grid grid-cols-[120px_1fr_1fr_1fr_1fr] gap-2 px-5 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50"
          >
            ...
            <span className={`font-mono text-sm font-bold ${row.Netto >= 0 ? 'text-green-600' : 'text-red-500'}`}>
              {fmt(row.Netto)}
            </span>
          </div>
        ))}
        <div className="grid grid-cols-[120px_1fr_1fr_1fr_1fr] gap-2 px-5 py-3 bg-gray-50 font-bold">
          ...
          <span className="font-mono text-sm text-green-600">{fmt(totInkomsten)}</span>
          <span className="font-mono text-sm text-red-500">{fmt(totUitgaven)}</span>
          <span className="font-mono text-sm">—</span>
          <span className={`font-mono text-sm ${totNetto >= 0 ? 'text-green-600' : 'text-red-500'}`}>
            {fmt(totNetto)}
          </span>
        </div>
      </div>
