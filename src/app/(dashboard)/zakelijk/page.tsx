...
export default function ZakelijkPage() {
  ...
  const chartData = MAANDEN.map((naam, i) => {
    const mTx = transacties.filter(t => new Date(t.date).getMonth() === i);
    return {
      naam,
      Omzet: Math.round(mTx.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0)),
      Kosten: Math.round(mTx.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0)),
    };
  });
    ...
    <div className="px-7 py-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Zakelijk overzicht</p>
        </div>
      </div>
      ...
      <div className="card p-6 mb-6">
        <h2 className="font-bold text-base mb-4">Maandoverzicht</h2>
      ...
      <div className="card p-6">
        ...
        {transacties.length === 0 ? (
          <p className="text-gray-400 text-sm text-center py-8">
            Markeer transacties als zakelijk in het transactieoverzicht
          </p>
        ) : (
